...
function ServicesContent() {
  ...
  useEffect(() => {
    const fetchServices = () => {
      setIsLoading(true);
      marketplaceApi
        .getStoreServices(subdomain, { page, limit: 12 })
        .then(({ data }) => {
    ...
    <StorefrontThemeProvider store={store || null}>
      <div className="mx-auto max-w-6xl px-4 py-8">
        ...
        {isLoading ? (
          ...
          <p className="text-sm text-gray-400">Belum ada jasa</p>
        ) : (
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {services.map((s: Service) => (
                ...
                >
                  ...
                  <p className="text-sm font-bold mt-1" style={{ color: store?.themeSecondary || store?.themeColor || 'rgb(16 185 129)' }}>
            ...
            {totalPages > 1 && (
              <div className="mt-6">
                <Pagination
                  ...
                  onPageChange={(p) =>
                    router.push(`${getStorefrontPath(subdomain, "/services")}?page=${p}`, {
                      scroll: false,
                    })
                  }
                />
