...
export default function StoreServiceDetailPage() {
  ...
  useEffect(() => {
    const fetchService = async () => {
      setIsLoading(true);
      try {
        const { data } = await marketplaceApi.getStoreServiceBySlug(subdomain, slug);
        ...
        setService(svc);
        setStoreName(svc.tenant?.name || subdomain);
    ...
    if (slug && subdomain) fetchService();
    ...
    if (!isAuthenticated) {
      ...
      router.push(
        `/login?returnUrl=${encodeURIComponent(
          getStorefrontPath(subdomain, `/services/${slug}`),
        )}`,
      );
      return;
    }
  ...
  if (!service) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-20 text-center">
        <Briefcase className="mx-auto mb-4 h-14 w-14 text-gray-200" />
        <h1 className="text-xl font-bold text-gray-900">Jasa Tidak Ditemukan</h1>
        <p className="mt-2 text-sm text-gray-500">
          Jasa ini tidak tersedia di toko {storeName || subdomain}.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-6">
      <nav className="mb-6 flex items-center gap-2 text-sm text-gray-500">
        ...
        <span>/</span>
        <Link href={getStorefrontPath(subdomain, "/services")} className="hover:text-blue-600">
          Jasa
        </Link>
        {service.category && (
          <>
            ...
            <div className="mt-6 border-t pt-6">
              ...
              <div className="space-y-3">
                {service.faq.map((item: { question: string; answer: string }, index: number) => (
                  <FAQAccordion key={index} question={item.question} answer={item.answer} defaultOpen={index === 0} />
                ))}
              </div>
            </div>
          )}
            ...
            {packages.length > 0 ? (
                ...
                {selectedPackage && (
                  <div className={`mt-4 rounded-xl border-2 p-5 ${TIER_COLORS[selectedPackage.tier] || "border-gray-200"}`}>
                    ...
                    <div className="mt-5 rounded-xl border border-emerald-100 bg-emerald-50/70 p-4 text-sm text-emerald-900">
                      Diskusikan kebutuhanmu langsung dengan seller untuk paket ini.
                    </div>
                  </div>
                )}
              </>
              ...
              <div className="rounded-xl border-2 border-gray-200 p-5">
                ...
                <div className="mt-4 rounded-xl border border-emerald-100 bg-emerald-50/70 p-4 text-sm text-emerald-900">
                  Layanan ini diproses melalui komunikasi langsung dengan seller.
                </div>
              </div>
            )}
          </div>
